...
static int q6asm_map_cal_memory(struct cal_block_data *cal_block)
{
	...
	if (result < 0) {
		...
		pr_debug("%s: mmap did not work! addr = 0x%pKa, size = %zd\n",
			__func__,
...
int send_asm_custom_topology(struct audio_client *ac)
{
	...
	asm_top.payload_size = cal_block->cal_data.size;
	 ...
	 pr_debug("%s: Sending ASM_CMD_ADD_TOPOLOGIES payload = %pKa, size = %d, map handle = 0x%x\n",
	...
	if (result < 0) {
		pr_err("%s: Set topologies failed result %d\n",
			__func__, result);
		pr_debug("%s: Set topologies failed payload = 0x%pKa\n",
			__func__, &cal_block->cal_data.paddr);
		goto unmap;
	...
	if (!result) {
		...
		pr_debug("%s: Set topologies failed after timedout payload = 0x%pKa\n",
			__func__, &cal_block->cal_data.paddr);
		result = -ETIMEDOUT;
		goto unmap;
	}
...
int q6asm_map_rtac_block(struct rtac_cal_block_data *cal_block)
{
	...
	if (result < 0) {
		pr_err("%s: mmap did not work! size = %d result %d\n",
			__func__,
			cal_block->map_data.map_size, result);
		pr_debug("%s: mmap did not work! addr = 0x%pKa, size = %d\n",
			__func__,
			&cal_block->cal_data.paddr,
			cal_block->map_data.map_size);
		goto done;
			...
			struct audio_client *ac)
{
	...
	if (port->buf[0].data) {
		pr_debug("%s: data[%pKK]phys[%pKa][%pKK] , client[%pKK] handle[%pKK]\n",
			__func__,
			port->buf[0].data,
			&port->buf[0].phys,
			&port->buf[0].phys,
		...
		while (cnt < bufcnt) {
			if (bufsz > 0) {
				if (!buf[cnt].data) {
					...
					pr_debug("%s: data[%pKK]phys[%pKa][%pKK]\n",
						__func__,
					   buf[cnt].data,
					   &buf[cnt].phys,
					   &buf[cnt].phys);
...
static int32_t q6asm_srvc_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == RESET_EVENTS) {
		pr_debug("%s: Reset event is received: %d %d apr[%pKK]\n",
				__func__,
				data->reset_event,
				data->reset_proc,
				this_mmap.apr);
...
static int32_t q6asm_callback(struct apr_client_data *data, void *priv)
{
	...
	if (!q6asm_is_valid_audio_client(ac)) {
		pr_err("%s: audio client pointer is invalid, ac = %pKK\n",
				__func__, ac);
		spin_unlock_irqrestore(
	...
	case ASM_DATA_EVENT_READ_DONE_V2:{
		...
		if (ac->io_mode & SYNC_IO_MODE) {
			...
			payload[READDONE_IDX_BUFADD_MSW]) {
				dev_vdbg(ac->dev, "%s: Expected addr %pKa\n",
				...
				uint32_t *index)
{
	...
	if (ac->io_mode & SYNC_IO_MODE) {
		...
		*index = port->cpu_buf;
		...
		dev_vdbg(ac->dev, "%s: session[%d]index[%d] data[%pKK]size[%d]\n",
						__func__,
						ac->session,
						port->cpu_buf,
						data, *size);
				...
				uint32_t passthrough_flag)
{
	...
	if (ac == NULL) {
		pr_err("%s: ac[%pKK] NULL\n",  __func__, ac);
		rc = -EINVAL;
		goto fail_cmd;
	}

	if (ac->apr == NULL) {
		pr_err("%s: APR handle[%pKK] NULL\n", __func__,  ac->apr);
		rc = -EINVAL;
		goto fail_cmd;
	}
			 ...
			 int dir)
{
	...
	open->topo_id = q6asm_get_asm_topology();
				...
				uint32_t bufsz, uint32_t bufcnt)
{
	...
	ac->port[dir].tmp_hdl = 0;
	port = &ac->port[dir];
	pr_debug("%s: buf_add 0x%pKa, bufsz: %d\n", __func__,
		&buf_add, bufsz);
				...
				bool is_contiguous)
{
	...
	q6asm_add_mmaphdr(ac, &mmap_regions->hdr, cmd_size, TRUE,
					((ac->session << 8) | dir));
	atomic_set(&ac->mem_state, 1);
	pr_debug("%s: mmap_region=0x%pKK token=0x%x\n", __func__,
	...
	for (i = 0; i < bufcnt; i++) {
		...
		pr_debug("%s: i=%d, bufadd[i] = 0x%pKa, maphdl[i] = 0x%x\n",
			...
			void *data, struct param_outband *po, int m_id)
{
	...
	if (!ac || ac->apr == NULL || (size == 0) || !data) {
		pr_err("DTS_EAGLE_ASM - %s: APR handle NULL, invalid size %u or pointer %pKK.\n",
			__func__, size, data);
		return -EINVAL;
	}
	...
	pr_debug("DTS_EAGLE_ASM - %s: ac %pKK param_id 0x%x size %u data %pKK m_id 0x%x\n",
	...
	if (po) {
		...
		pr_debug("DTS_EAGLE_ASM - %s: using out of band memory (virtual %pKK, physical %lu)\n",
			...
			int len)
{
	...
	if (ac->io_mode & SYNC_IO_MODE) {
		...
		dev_vdbg(ac->dev, "%s: session[%d]dsp-buf[%d][%pKK]cpu_buf[%d][%pKa]\n",
				__func__,
				ac->session,
				dsp_buf,
		...
		port->dsp_buf = q6asm_get_next_buf(ac, port->dsp_buf,
						   port->max_buf_cnt);
		mutex_unlock(&port->lock);
		dev_vdbg(ac->dev, "%s: buf add[%pKa] token[%d] uid[%d]\n",
					  ...
					  struct audio_aio_write_param *param)
{
	...
	dev_vdbg(ac->dev, "%s: token[0x%x], buf_addr[%pKa], buf_size[0x%x], ts_msw[0x%x], ts_lsw[0x%x], lbuf_phys_addr: 0x[%pKa]\n",
			__func__,
		...
		uint32_t lsw_ts, uint32_t flags)
{
	...
	if (ac->io_mode & SYNC_IO_MODE) {
		...
		dev_vdbg(ac->dev, "%s: ab->phys[%pKa]bufadd[0x%x] token[0x%x]buf_id[0x%x]buf_size[0x%x]mmaphdl[0x%x]"
				, __func__,
				&ab->phys,
				write.buf_addr_lsw,
				write.hdr.token,
